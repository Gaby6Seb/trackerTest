...
function filterDataForUser(fullData, userFilterConfig) {
	...
	const result = {
		...
		stealth: processPlayers(fullData.stealth),
		safe: processPlayers(fullData.safe)
	};
	if (!userFilterConfig.canSeeLastKnownLocation) {
		...
		result.stealth = result.stealth.map(stripCoords);
		result.safe = result.safe.map(stripCoords);
	}
	return result;
}
...
function processNotifications(fullData) {
	...
	[...fullData.notLocated, ...fullData.located, ...fullData.safe, ...fullData.stealth].forEach(p => allPlayersMap.set(p.u, p));
	...
	allPlayersMap.forEach((p, uid) => {
		...
		if (fullData.safe.some(pl => pl.u === uid)) status = 'safe'; // updated
		if (fullData.stealth.some(pl => pl.u === uid && !p.isImmune)) status = 'stealth'; // updated
	...
	io.sockets.sockets.forEach(socket => {
		...
		newPlayerStatusMap.forEach((currentStatus, playerId) => {
			...
			if (previousStatus === 'located' && (currentStatus === 'stealth' || currentStatus === 'notLocated')) {
...
async function runApiRequests() {
	...
	try {
		...
		// ####################################################################
		// ### FIX: Revised Player Categorization to prioritize Immunity    ###
		// ####################################################################
		...
		locationResponse.data.forEach(locData => {
			...
			// Base player object that will be categorized
			const playerObject = {
				u: locData.u,
			...
			// Add coordinate data if available (live or last known)
			if (hasCoords) {
				...
				if (lastKnown) {
					Object.assign(playerObject, lastKnown);
				}
			}

			// Categorize players based on a clear hierarchy: Immunity > Safe Zone > Located > Stealthed
			if (isImmune) {
				safePlayers.push(playerObject);
			} else if (isInGeographicSafeZone) {
				safePlayers.push(playerObject);
			} else if (hasCoords) {
				locatedPlayers.push(playerObject);
			} else {
				stealthedPlayers.push(playerObject);
			}
		});
		...
		// Stealth Timer Fetching (Now operates on stealthedPlayers)
		const currentStealthedIds = new Set(stealthedPlayers.map(p => p.u));
		for (const player of stealthedPlayers) {
			// No need to check for immunity here, as they are already filtered out
		...
		lastSuccessfulData = {
			...
			notLocated: notLocatedPlayers, // This remains empty but is kept for schema consistency
			stealth: stealthedPlayersWithTimers,
			safe: safePlayers,
		};
		io.sockets.sockets.forEach(socket => {
